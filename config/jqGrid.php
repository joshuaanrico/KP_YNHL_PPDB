...
require_once 'jqCalendar.php';
class jqGrid
{
    public $version = '4.3.0.0';
    protected $pdo;
    protected $I = '';
    protected $dbtype;
    protected $select = "";
    ...
    public function sendErrorHeader()
    {
        ...
        if ($this->customClass) {
            try {
                $this->errorMessage = call_user_func(array(
                    $this->customClass,
                    $this->customError
                ), $this->oper, $this->errorMessage);
            }
            catch (Exception $e) {
...
class jqGridEdit extends jqGrid
{
        ...
        switch ($oper) {
            case 'add':
                ...
                if ($acnt > 0) {
                    for ($i = 0; $i < $acnt; $i++) {
                        if ($this->debug)
                            $this->logQuery($ar[$i]['sql'], $ar[$i]['params']);
                        $stmt   = jqGridDB::prepare($this->pdo, $ar[$i]['sql'], $ar[$i]['params']);
                        $result = jqGridDB::execute($stmt, $ar[$i]['params']);
    ...
    public function setBeforeCrudAction($oper, $sql, $params = null)
    {
        switch ($oper) {
            case 'add':
                $this->_addarrayb[] = array(
                    "sql" => $sql,
                    "params" => $params
                );
                break;
            case 'edit':
                $this->_editarrayb[] = array(
                    "sql" => $sql,
                    "params" => $params
                );
                break;
            case 'del':
                $this->_delarrayb[] = array(
                    "sql" => $sql,
                    "params" => $params
                );
                break;
...
SUBGRID;
    ...
    public function setSelect($colname, $data, $formatter = true, $editing = true, $seraching = true, $defvals = array(), $sep = ":")
    {
        $s1    = "";
        $prop  = array();
            ...
            if ($colname) {
                if ($ac->isNotACQuery()) {
                    ...
                    if ($editing) {
                        ...
                        $this->setColProperty($colname, array(
                            "editoptions" => array(
                                "dataInit" => "js:function(el){" . $script . "}"
                            )
                        ));
                    }
                    if ($searching) {
    ...
    public function setColModel(array $model = null, array $params = null, array $labels = null)
    {
        ...
        if ($runme) {
            ...
            if ($ret) {
                ...
                for ($i = 0; $i < $colcount; $i++) {
                    ...
                    if ($names && array_key_exists($meta['name'], $labels))
                        $this->colModel[] = array(
                            ...
                            'name' => $meta['name'],
                            'index' => $meta['name'],
                            'sorttype' => jqGridDB::MetaType($meta, $this->dbtype)
                        );
                    else
                        $this->colModel[] = array(
                            'name' => $meta['name'],
                ...
                if ($pk) {
                    $this->setColProperty($pk, array(
                        "key" => true
                    ));
                } else {
                    $this->colModel[0] = array_merge($this->colModel[0], array(
                        "key" => true
                    ));
                }
            } else {
    ...
    public function renderGrid($tblelement = '', $pager = '', $script = true, array $summary = null, array $params = null, $createtbl = false, $createpg = false, $echo = true)
    {
        ...
        if ($goper == $this->GridParams["autocomplete"]) {
            ...
            if (isset($this->gridOptions['postData']))
                $this->gridOptions['postData'] = jqGridUtils::array_extend($this->gridOptions['postData'], array(
                    $oper => $this->GridParams["query"]
                ));
            else
                $this->setGridOptions(array(
                    "postData" => array(
            ...
            if (!isset($this->editOptions['mtype']) && $this->showError) {
                $this->setNavEvent('edit', 'afterSubmit', "function(res,pdata){ var result = res.responseText.split('##'); if(result[0]=='success') return [true,result[1],result[2]]; else return [false,result[1],''];}");
            }
            if (!isset($this->addOptions['mtype']) && $this->showError) {
                $this->setNavEvent('add', 'afterSubmit', "function(res,pdata){ var result = res.responseText.split('##'); if(result[0]=='success') return [true,result[1],result[2]]; else return [false,result[1],''];}");
            }
            if (strlen($pager) > 0)
                $this->setGridOptions(array(
                    "pager" => $pager
...
AFTERS;
                $this->inlineNavOpt['addParams']  = jqGridUtils::array_extend($this->inlineNavOpt['addParams'], array(
                    "aftersavefunc" => "js:" . $aftersave
                ));
                $this->inlineNavOpt['editParams'] = jqGridUtils::array_extend($this->inlineNavOpt['editParams'], array(
                    "aftersavefunc" => "js:" . $aftersave
                ));
